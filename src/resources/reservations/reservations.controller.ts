...
reservationsController.post("/", validateDTO(CreateReservationDTO), async (req, res) => {
    try {
        const result = await ReservationsService.addReservation(res.locals.dtoInstance);
...
reservationsController.get("/", async (req, res) => {
    const data = await ReservationsService.getReservations();
    res.status(StatusCodes.OK).send(data);
});

reservationsController.get("/:id", async (req, res) => {
    try {
        const data = await ReservationsService.getReservationDetails(req.params.id);
        res.status(StatusCodes.OK).send(data);
    } catch {
...
reservationsController.get("/filter", async (req, res) => {
...
reservationsController.patch("/:id", validateDTO(UpdateReservationDTO), async (req, res) => {
    try {
